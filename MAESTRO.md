...
16. **NINGÚN ERROR SIN ACCIÓN (UX, vinculante):** todo mensaje de error o prerrequisito del Studio nombra exactamente lo que falta Y ofrece ahí mismo el botón que lo resuelve. Ningún error se muestra solo como texto plano sin salida accionable. Toda operación >3s muestra progreso con etapa nombrada y tiempo transcurrido. Precedente: sesión 22, flujo sin callejones.
17. **PRINCIPIO DE PRODUCTO (decisión del arquitecto, s26, vinculante):** CENTRITO ES UNA HERRAMIENTA GENERAL DE VIDEO. Ningún tipo de fuente es "fuera de dominio" por diseño: podcasts multicámara, clases, grabaciones de pantalla, celular, material ya vertical, baja resolución. Cuando un tipo aún no está soportado, el sistema lo DETECTA, lo DICE y ofrece la ruta más cercana (fallback digno) — nunca produce basura en silencio. Norte del reframe: modo AUTO que elige la ruta correcta por video y por segmento. Los racionales previos que justificaban decisiones con "el material de K son clases OBS" quedan actualizados: eso describe UNA fuente frecuente, no el dominio del producto.
18. **Tests de tracking con aserciones intermedias (s24/s26):** las aserciones deben verificar la SALIDA DE CADA MUESTRA INTERMEDIA, no solo el estado final. Precedente: el bug de identidad del proyecto de referencia (2 frames de identidad errónea) era invisible a aserciones de estado final y solo se detectó frame a frame. (Registrada en s26; propuesta original en PREGUNTAS #26e.)
19. **DOS MODOS, UN MOTOR (decisión del arquitecto, s27, vinculante):** (a) MODO AUTOMÁTICO: el usuario sube un video, elige objetivo y recibe un paquete listo para revisar y publicar — es una CAPA DELGADA que orquesta las herramientas existentes (jobs/CLIs); jamás reimplementa pipeline. (b) MODO CREADOR: las herramientas actuales del Studio, separadas, con control granular — nada se esconde ni se elimina (regla 15). Toda función nueva nace como herramienta del motor, usable desde ambos modos. PROHIBIDO: duplicar lógica en la capa automática; convertir Centrito en editor genérico de timeline — Centrito es una fábrica con opinión (compite con OpusClip/Captions), no un editor. El paquete final SIEMPRE incluye paso de revisión humana antes de publicar (insight de K, s27; conecta con la aprobación de F7). Roadmap de la capa: PREGUNTAS #29.
20. **COMMIT POR BLOQUE TERMINADO (proceso, s27, vinculante — del incidente de cierre de ventana):** al completar cada bloque de sesión con tests verdes, se hace `/revisar` + commit ANTES de iniciar el siguiente. Las corridas largas de validación (renders, transcripciones, autopiloto sobre material real) van SIEMPRE después de sellar el código. Un cierre de ventana nunca debe costar más que la última corrida. Corolario para el Modo Automático: los procesos largos son reanudables — cada artefacto caro ya generado (transcript, análisis del clipper, clip renderizado) es un checkpoint que no se vuelve a pagar.
