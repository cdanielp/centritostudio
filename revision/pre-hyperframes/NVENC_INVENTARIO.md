...
| `reframe.py` | `_cmd_ffmpeg_pipe` / `renderizar_reframe` | pipe rawvideo bgr24 → encode | libx264 | fast/crf18 | `-c:a copy` | **Sí (cat 3)** | Alto (pipe OpenCV, faststart) | **Integrado** (selección FAST + fallback re-pipe; pix_fmt sin duplicar) |
...
| `submagic.py` | render nube | API remota | — | — | — | No (cat 6) | — | Fuera de alcance (no codifica local) |
...
`clipper` y `Auto` **no** reimplementan encode: heredan `depurador.run_edl` y
`core.burn_video_with_emojis`. Cada worker captura una **instantánea inmutable** del modo
(`@video_encoder.con_snapshot`) para que un cambio de preferencia a mitad de un job no lo altere.
