...
| P0-2 | **P0** | `app.py:151` `upload_video` filename crudo + sin límite | Upload | **CERRADO en H1, pendiente merge** (basename+ext+tope+tmp+ffprobe) | no | sí | H1 |
...
| P1-OUT-2 | P1 | `core_ass.py:328` FFmpeg escribe al nombre final | Render/resume | **CERRADO en H1, pendiente merge** (tmp `*.part-<uuid>.mp4` + `os.replace`) | no | sí | H1 |
