...
def _new_job(description: str) -> str:
    ...
    with _JOBS_LOCK:
        _JOBS[jid] = {"status": "pending", "progress": 0, "message": description, "result": None, "error": None}
    return jid
...
def start_render(name: str, style: str = "hormozi",
    ...
    if style not in STYLES:
        raise HTTPException(400, f"Estilo inválido. Opciones: {', '.join(STYLES)}")

    jid = _new_job(f"Renderizando {name} en {style}...")
    threading.Thread(
        target=_run_render,
        args=(jid, mp4, grp_path, name, style, words_per_group),
        daemon=True,
    ).start()
    return {"job_id": jid}


def _run_render(jid: str, mp4: Path, grp_path: Path,
                name: str, style: str, words_per_group: int | None):
    try:
        ...
        # Re-agrupar si se pide --words-per-group distinto al guardado
        ...
        out_path = OUTPUT_DIR / f"{name}_{style}.mp4"

        _update_job(jid, progress=35, message="Generando subtítulos ASS...")
        ...
        _update_job(jid, status="done", progress=100,
                    ...
                    result={"output": out_path.name, "elapsed": elapsed})
    except Exception as exc:
        _update_job(jid, status="error", message=str(exc), error=str(exc))
